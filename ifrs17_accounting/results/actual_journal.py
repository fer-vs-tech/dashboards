...
def get_data(
    ...
    mapping_db = db_helper.ActualCFJournal(journal_type=journal_type)
    ...
    if wvr_path is not None:
        ...
        df = helpers.get_df(mapping_db, wvr_path, "mapping")
        filter_zeros = False
    else:
        ...
        df = helpers.get_dataframe(mapping_db, mapping_connection)
        filter_zeros = True

    df = helpers.populate_journal_data(
        df,
        company_id,
        journal_if_df,
        journal_nb_df,
