...
def set_conditional_style_by_filtering(column, filter_value=0, color="#eb5757"):
    """
    Set the conditional style for certain columns
    :param column: column name to be filtered by value (str) or list of column names
    ...
    if isinstance(column, list):
        for col in column:
            condition = {
                "if": {
                    "filter_query": f"{{{col}}} < '{filter_value}'",
                    "column_id": col,
                },
                "backgroundColor": "#f8e6ec",
                "color": color,
