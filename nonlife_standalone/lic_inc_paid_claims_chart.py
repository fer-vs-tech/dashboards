...
def get_chart(wvr_path, title):
    ...
    """
    handler = db_helper.IncPaidClaimsHandler()
    chart_data_df = handler.get_wvr_data(wvr_path, "NL_LIC_Model")
    # print(chart_data_df)

    try:
        chart = init_chart(title, chart_data_df, "Incremental_Paid_Claims")
    except:
    ...
    hovertemp = "<b>Dev_Period_Position: </b> %{x} <br>"
    hovertemp += "<b>Incremental_Paid_Claims Bill: </b> %{y}"
    chart.update_traces(hovertemplate=hovertemp)

    table_data_df = dash_utils.pivot_data(
        chart_data_df,
        "Dev_Period_Position",
        "Origin_Period_Position",
        "Incremental_Paid_Claims",
    )
    # print(table_data_df)
