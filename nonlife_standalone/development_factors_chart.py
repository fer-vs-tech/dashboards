...
def get_chart(wvr_path, title):
    ...
    """
    handler = db_helper.DevelopmentFactorsPostData()
    chart_data_df = handler.get_wvr_data(wvr_path, "NL_LIC_Model")
    # print(chart_data_df)

    try:
        chart = init_chart(title, chart_data_df, "Dev_Factors")
    except:
    ...
    columns = dash_utils.set_column_names(table_data_df.columns, precision=3)
