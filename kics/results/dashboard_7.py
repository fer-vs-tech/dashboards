...
def get_table_data(wvr_path, model_name, report_date):
    ...
    journal_code = "7"
    data = db_helper.OperationRisk(report_date=report_date, name=KICS_NAME)
    ...
    template_df, _ = helpers.get_template_df(
        journal_code, generate_header_rows=False, header=header, kics_name=KICS_NAME
    )
    ...
    results = helpers.prepare_table_data(
        template_df, multi_index=True, show_negative_numbers=True, precision=1
    )
