...
app = dash.Dash(
    name="genledger",
    server=server,
    url_base_pathname="/dash/genledger/",
...
app.layout = html.Div(
    children=[
        ...
        html.H2(children="IFRS17 General ledger", style={"color": "darkblue"}),
        html.Br(),
...
def general_ledger_callback(jobrun):
    """
    Update the table listing database schemas
    """
    handler = gl.GenLedgerHandler()
    pop_template = handler.get_combined_ledger(WVR_PATH)
