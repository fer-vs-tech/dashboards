...
def get_data(wvr_path, company_id, journal_type):
    ...
    # Get total credit and debit for each journal
    mapping_db = db_helper.ActualCFMapping(journal_type=journal_type)
    ...
    if journal_df.shape[0] == 0 and journal_df.shape[0] == 0:
        raise Exception(
            f"No data found for {journal_type} journal with company_id {company_id}"
        )

    mapping_df = helpers.get_df(mapping_db, wvr_path, model="mapping")

    # Add neededmapping_df columns to df
    ...
    mapping_df["COHT"] = mapping_df.apply(
        lambda _: helpers.set_value(journal_df, "COHT", apply_formatter=False), axis=1
    )
